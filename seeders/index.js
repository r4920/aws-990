...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'kLU0HRNmdr87gBR',
      'username':'Eunice.Hickle91',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Eunice.Hickle91' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'ydwQZuDSS7zRdpS',
      'username':'Marion.Runte0',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Marion.Runte0' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
